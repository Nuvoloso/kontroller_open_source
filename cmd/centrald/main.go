...
import (
	...
	"github.com/Nuvoloso/kontroller/pkg/centrald/audit"
	"github.com/Nuvoloso/kontroller/pkg/centrald/auth"
	_ "github.com/Nuvoloso/kontroller/pkg/centrald/handlers" // load handlers
	_ "github.com/Nuvoloso/kontroller/pkg/centrald/heartbeat"
	appMetrics "github.com/Nuvoloso/kontroller/pkg/centrald/metrics"
	appDS "github.com/Nuvoloso/kontroller/pkg/centrald/mongods"
	appSReq "github.com/Nuvoloso/kontroller/pkg/centrald/sreq"
	appVReq "github.com/Nuvoloso/kontroller/pkg/centrald/vreq"
...
type mainContext struct {
	...
	AuthCfg *auth.Config `group:"Authentication and Authorization Options" namespace:"auth"`
	...
	AuditLogArgs audit.Args               `group:"Audit Log Options" namespace:"audit"`
	MongoArgs    appDS.MongoDataStoreArgs `group:"Mongo Options" namespace:"mongo"`
	MetricsArgs  appMetrics.Args          `group:"Metrics Options" namespace:"metrics"`
...
func main() {
	...
	parser.LongDescription = "The central Nuvoloso management service.\n\n" +
	...
	// initialize the back end and install components
	evMArgs := &crude.ManagerArgs{ // TBD: may contain flag args in future
		WSKeepAlive: true, // required with nginx proxy
		Log:         mCtx.Log,
	}
	evM := crude.NewManager(evMArgs)
	appSReq.PoolInit(&mCtx.SReqArgs)
	appVReq.ComponentInit(&mCtx.VReqArgs)
	audit.Register(&mCtx.AuditLogArgs)
	appMetrics.Register(&mCtx.MetricsArgs)
	mCtx.MongoArgs.Log = mCtx.Log
	mCtx.MongoArgs.AppName = Appname
	mCtx.MongoArgs.TLSCertificate = string(server.TLSCertificate)
	mCtx.MongoArgs.TLSCertificateKey = string(server.TLSCertificateKey)
	mCtx.MongoArgs.TLSCACertificate = string(server.TLSCACertificate)
	mCtx.AppArgs.DS = appDS.NewDataStore(&mCtx.MongoArgs)
	...
	mCtx.AuthCfg.DebugClient = mCtx.AppArgs.DebugClient
	mCtx.AuthCfg.Log = mCtx.Log
	mCtx.AuthCfg.DS = mCtx.AppArgs.DS
	mCtx.AuthCfg.TLSCertificate = string(server.TLSCertificate)
	mCtx.AuthCfg.TLSCertificateKey = string(server.TLSCertificateKey)
	mCtx.AuthCfg.TLSCACertificate = string(server.TLSCACertificate)

	// Setup middleware
	server.SetHandler(mCtx.outermostMiddleware(
		api.Serve(func(next http.Handler) http.Handler {
			return util.NumberRequestMiddleware(mCtx.AuthCfg.Middleware(mCtx.loggingHandler(evM.CollectorMiddleware(mCtx.innermostMiddleware(next)))))
		})))
