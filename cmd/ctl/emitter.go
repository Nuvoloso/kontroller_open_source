...
// Reset destroys cached encoders
func (e *StdoutEmitter) Reset() {
	e.je = nil
	e.cnt = 0
}
...
func (e *StdoutEmitter) EmitJSON(data interface{}) error {
	if e.je == nil {
		e.je = json.NewEncoder(outputWriter)
		e.je.SetIndent("", "    ")
		e.je.SetEscapeHTML(false)
	}
	e.cnt++
	return e.je.Encode(data)
}

// EmitYAML emits YAML to stdout
// Multiple documents will be separated.
func (e *StdoutEmitter) EmitYAML(data interface{}) (err error) {
	...
	fmt.Fprintf(outputWriter, "%s%s", sep, string(b))
	return nil
}
